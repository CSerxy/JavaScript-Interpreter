...
我需要这个函数做以下事情：
1. 接受一个string，以这个string为开始进行处理，处理内容的范围为以startChar为开始的字符, 以endChar为截止的字符。在这里需要注意读了新的字符串之后如果遇到了截止字符串，则该函数结束。你需要把截止字符之前的东西截断，把未处理的字符串部分放入objReturn中的next字符串中（这样做的原因是这部分未处理的字符串我之后需要用到）。
